...
const selectCategory = (category: string) => {
  router.replace({ query: { ...route.query, category: category.toLowerCase() } })
}
</script>
  ...
  <transition-root appear :show="isOpen" as="template">
    <Dialog as="div" class="relative z-[70]" @close="$emit('close')">
      ...
      >
        ...
        >
          <div class="space-y-1 flex items-center justify-between border-b p-4 dark:border-gray-700 md:p-5">
          ...
          <div class="flex justify-start flex-col px-4 space-y-6 my-8 dark:bg-gray-900 dark:text-gray-200">
            <div
              v-for="category of categories"
              :key="category"
              class="text-md font-normal cursor-pointer"
              :to="{ query: { category } }"

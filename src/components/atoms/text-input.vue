  ...
  <div class="flex w-full flex-col border-none bg-transparent p-0">
    ...
    <div class="relative">
      ...
      <input
        ...
        class="block w-full rounded border border-gray-200 py-2.5 text-sm transition placeholder:font-light placeholder:text-gray-300 focus:border-gray-900 focus:ring-0 focus:outline-none disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-400 disabled:opacity-75 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:placeholder:text-gray-500 dark:focus:border-gray-400"
        @input="updateValue"
      />
    ...
    <div v-if="props.errors.length > 0" class="text-error h-4 py-1 text-xs font-light tracking-normal">
      <p>{{ props.errors[0] }}</p>
    </div>
  </div>
</template>
